...
def test_optional_ml_summary_never_blends_config_pairs_or_models():
    paired = pd.DataFrame({
        "baseline_config": ["A", "A", "B"], "comparison_config": ["B", "B", "C"],
        "model": ["LR", "RF", "LR"], "metric": ["auc"] * 3,
        "baseline_value": [0.4, 0.5, 0.6], "comparison_value": [0.5, 0.7, 0.9], "delta": [0.1, 0.2, 0.3],
    })
    summary = claim_writer.optional_robustness_summary({"ml_paired": paired})
    assert len(summary["ml_metrics"]) == 3
    assert {row["model"] for row in summary["ml_metrics"].values()} == {"LR", "RF"}
    assert {row["baseline_config"] for row in summary["ml_metrics"].values()} == {"A", "B"}


def _complete_manifest_artifacts(run: Path, inference: Path, inference_hash: str | None = None) -> dict:
    names = {
        "code": [
    ...
    sections = {}
    for section, filenames in names.items():
        entries = []
        for filename in filenames:
            path = inference if filename == inference.name else run / filename
            if path != inference:
                path.write_text(f"test artifact: {filename}\n", encoding="utf-8")
            digest = inference_hash if path == inference and inference_hash is not None else claim_writer.sha256_file(path)
            entries.append({"path": str(path), "sha256": digest})
        sections[section] = entries
    return sections


def test_harmonized_reader_requires_same_row_gate_and_restricts_pilot_claim(tmp_path, monkeypatch):
    monkeypatch.setattr(claim_writer, "validate_harmonized_manifest_artifacts", lambda *_args: (True, True, True))
    run = tmp_path / "pilot_v1"; run.mkdir()
    inference = pd.DataFrame({
        "family": ["P1", "P1"],
        "baseline_config": ["B_technical_coverage_keyword", "B_technical_coverage_keyword"],
        "comparison_config": ["C_technical_coverage_semantic", "C_technical_coverage_semantic"],
        "model": ["RandomForest", "LogisticRegression"],
        "metric": ["balanced_accuracy", "balanced_accuracy"],
        "status": ["ok", "ok"], "p_value_bh": [0.01, 0.20],
        "bootstrap_ci_low": [0.1, 0.2],
    })
    inference.to_csv(run / "harmonized_inference.csv", index=False)
    manifest = {
        "mode": "pilot", "run_id": "pilot_v1", "claim_level": "common_stratified_article_spine_only",
        "status": "completed", "run_tier": "canonical", "coverage_audit_pass": True,
        "fold_audit_pass": True, "primary_gate_any": True, "thesis_evidence_gate": True,
        **_complete_manifest_artifacts(run, run / "harmonized_inference.csv"),
    }
    (run / "harmonized_comparison_manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    summary = claim_writer.harmonized_run_summary(run)
    assert summary["primary_gate_pass"]
    manifest["run_tier"] = "smoke"
    manifest["thesis_evidence_gate"] = False
    (run / "harmonized_comparison_manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    smoke = claim_writer.harmonized_run_summary(run)
    assert smoke["status"] == "smoke_not_evidence" and not smoke["primary_gate_pass"]
    manifest["run_tier"] = "canonical"
    manifest["thesis_evidence_gate"] = True
    manifest["claim_level"] = "full_corpus"
    (run / "harmonized_comparison_manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    blocked = claim_writer.harmonized_run_summary(run)
    assert blocked["status"] == "invalid_claim_level" and not blocked["primary_gate_pass"]


def test_harmonized_reader_fails_closed_on_audit_and_gate_mismatch(tmp_path, monkeypatch):
    monkeypatch.setattr(claim_writer, "validate_harmonized_manifest_artifacts", lambda *_args: (True, True, True))
    run = tmp_path / "canonical"; run.mkdir()
    pd.DataFrame({
        "family": ["P1"],
        ...
        "status": ["ok"], "p_value_bh": [0.01], "bootstrap_ci_low": [0.1],
    }).to_csv(run / "harmonized_inference.csv", index=False)
    manifest = {
        "mode": "sample500", "run_id": "canonical", "claim_level": "balanced_validation_sample_only",
        "status": "completed", "run_tier": "canonical", "coverage_audit_pass": False,
        "fold_audit_pass": True, "primary_gate_any": True, "thesis_evidence_gate": True,
        **_complete_manifest_artifacts(run, run / "harmonized_inference.csv"),
    }
    path = run / "harmonized_comparison_manifest.json"
    path.write_text(json.dumps(manifest), encoding="utf-8")
    assert claim_writer.harmonized_run_summary(run)["status"] == "audit_gate_failed"
    manifest["coverage_audit_pass"] = True
    manifest["primary_gate_any"] = False
    path.write_text(json.dumps(manifest), encoding="utf-8")
    assert claim_writer.harmonized_run_summary(run)["status"] == "gate_mismatch"


def test_harmonized_reader_missing_and_hash_mismatch_fallback(tmp_path):
    assert claim_writer.harmonized_run_summary(tmp_path)["status"] == "missing_or_incomplete"
    run = tmp_path / "r"; run.mkdir()
    inference = run / "harmonized_inference.csv"
    pd.DataFrame({
        "family": ["P1"],
        "baseline_config": ["B_technical_coverage_keyword"],
        "comparison_config": ["C_technical_coverage_semantic"],
    ...
    manifest = {
        "mode": "sample500", "run_id": "r", "claim_level": "balanced_validation_sample_only",
        **_complete_manifest_artifacts(run, inference, "wrong"),
    }
    (run / "harmonized_comparison_manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    assert claim_writer.harmonized_run_summary(run)["status"] == "manifest_artifact_hash_mismatch"
    manifest.pop("code")
    (run / "harmonized_comparison_manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    assert claim_writer.harmonized_run_summary(run)["status"] == "incomplete_manifest_artifacts"
    manifest.update(_complete_manifest_artifacts(run, inference))
    manifest["outputs"] = manifest["outputs"][1:]
    (run / "harmonized_comparison_manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    assert claim_writer.harmonized_run_summary(run)["status"] == "incomplete_manifest_artifacts"


def test_report_clis_accept_explicit_harmonized_run_dir():
    for script in ("write_claim_evidence_table.py", "write_final_reports.py"):
        result = subprocess.run(
            [sys.executable, str(ROOT / "multi_llm_evidence_extraction" / "scripts" / script), "--help"],
            cwd=ROOT, capture_output=True, text=True,
        )
        assert result.returncode == 0, result.stderr
        assert "--harmonized-run-dir" in result.stdout


def test_lineage_discovery_always_includes_harmonized_config():
    assert lineage.HARMONIZED_CONFIG in lineage.discover_harmonized_artifacts()
