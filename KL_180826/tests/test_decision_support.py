...
def test_local_router_requires_loopback_and_approved_model(monkeypatch):
    monkeypatch.setenv("URL_LOCAL", "https://example.test/v1")
    monkeypatch.setenv("API_LOCAL", "secret")
    with pytest.raises(llm_provider.ProviderConfigError, match="localhost"):
        llm_provider.make_llm_client("local_router")

    monkeypatch.setenv("URL_LOCAL", "http://127.0.0.1:20128/v1")
    assert llm_provider.resolve_model("local_router") == llm_provider.DEFAULT_LOCAL_ROUTER_MODEL
    with pytest.raises(llm_provider.ProviderConfigError, match="not approved"):
        llm_provider.resolve_model("local_router", "unapproved-model")


def test_local_router_generation_and_score_use_loopback_only(monkeypatch):
    monkeypatch.setenv("URL_LOCAL", "http://localhost:20128/v1")
    monkeypatch.setenv("API_LOCAL", "router-secret")
    calls = []
    ...
    class FakeRequests:
        ...
        def post(url, headers, json, timeout):
            calls.append({"url": url, "headers": headers, "json": json, "timeout": timeout})
            content = '{"overall": 5}' if json.get("response_format") else "router text"
            return FakeDeepSeekResponse({
                "id": "router-1",
                "model": "router-response-model",
                "choices": [{"message": {"content": content}, "finish_reason": "stop"}],
            })

    client, _ = llm_provider.make_llm_client("local_router")
    generated = llm_provider.call_generate(
        "local_router", client, FakeRequests, llm_provider.DEFAULT_LOCAL_ROUTER_MODEL, "system", "prompt", 100, "high"
    )
    scored = llm_provider.call_score(
        "local_router", client, FakeRequests, llm_provider.DEFAULT_LOCAL_ROUTER_MODEL, "system", "prompt", 100, "high", {"type": "object"}
    )

    assert generated["text"] == "router text"
    assert generated["route_provenance"]["endpoint_host"] == "localhost:20128"
    assert scored["parsed"] == {"overall": 5}
    assert all(call["url"] == "http://localhost:20128/v1/chat/completions" for call in calls)
    assert "router-secret" not in json.dumps(generated)
    assert "router-secret" not in json.dumps(scored)
