...
----------------------
This quarterly ``label_basic`` workflow is retained for historical/exploratory
reproduction only. Its inner news merge filters out no-news quarters, and its
configuration names do not match the active V6 primary protocol: ``Config_B``
is keyword-only here (not Technical+Keyword), while ``Config_C`` is
Technical+Keyword (not Technical+Semantic). V6 H1/H2 must be evaluated through
``multi_llm_evidence_extraction/scripts/run_v6_technical_primary.py``.
"""
