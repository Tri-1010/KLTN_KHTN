...
def render_main_report(snapshot: dict[str, Any]) -> str:
    ...
    harmonized = snapshot.get("harmonized", {"available": False, "status": "missing"})
    ...
    return "\n".join([
        ...
        "",
        f"Available={harmonized.get('available')}; status={harmonized.get('status')}; mode={harmonized.get('mode')}; run={harmonized.get('run_id')}; claim level={harmonized.get('claim_level')}; same-row primary gate pass={harmonized.get('primary_gate_pass')}. Pilot and sample500 runs remain validation-sample evidence and cannot produce full-corpus claims.",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write semantic-news study reports from structured artifacts.")
    parser.add_argument("--harmonized-run-dir", type=Path)
    args = parser.parse_args()
    ensure_dirs()
    snapshot = build_artifact_snapshot(harmonized_run_dir=args.harmonized_run_dir)
