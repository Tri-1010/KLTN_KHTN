...
def build_session_aligned_targets(
    prices: pd.DataFrame,
    benchmark: pd.DataFrame,
    horizon: int = HORIZON,
) -> pd.DataFrame:
    """Build stock/benchmark returns on identical benchmark entry/exit sessions.

    Column names follow ``T{horizon}``. Use this for V6 horizon sensitivity.
    The frozen harmonized/canonical protocol must keep calling
    ``build_harmonized_targets`` (T+20 only).
    """
    if int(horizon) < 1:
    ...
    stock_col = f"stock_return_T{horizon}"
    bench_col = f"VNINDEX_return_T{horizon}"
    excess_col = f"excess_return_T{horizon}"
    label_col = f"label_outperform_T{horizon}"
    out[stock_col] = pd.NA
    out[bench_col] = pd.NA
    out[excess_col] = pd.NA
    out.loc[ok, stock_col] = out.loc[ok, "stock_exit_close"] / out.loc[ok, "stock_entry_close"] - 1
    out.loc[ok, bench_col] = out.loc[ok, "benchmark_exit_close"] / out.loc[ok, "benchmark_entry_close"] - 1
    out.loc[ok, excess_col] = out.loc[ok, stock_col] - out.loc[ok, bench_col]
    out[label_col] = pd.Series(pd.NA, index=out.index, dtype="Int64")
    out.loc[ok, label_col] = (pd.to_numeric(out.loc[ok, excess_col]) > 0).astype("Int64")
    out["artifact_schema_version"] = (
        HARMONIZED_SCHEMA_VERSION if horizon == HORIZON else f"session_aligned_targets_T{horizon}_v1"
    )
    if out.duplicated(["ticker", "entry_date"]).any():
        raise ValueError("duplicate session-aligned target keys")
...
def build_harmonized_targets(
    ...
    """Frozen harmonized/canonical T+20 builder. Non-20 horizons are rejected."""
    if horizon != HORIZON:
        raise ValueError(f"harmonized protocol requires horizon={HORIZON}")
    return build_session_aligned_targets(prices, benchmark, horizon=HORIZON)
