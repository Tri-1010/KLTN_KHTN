...
def validate_safe_identifier(value: str, field: str = "identifier") -> str:
    text = norm(value)
    if text in {"", ".", ".."} or not SAFE_IDENTIFIER_PATTERN.fullmatch(text):
        raise ValueError(f"unsafe {field}: {value!r}; use only letters, digits, dot, underscore, hyphen")
    return text


def resolve_scoped_path(root: Path, identifier: str) -> Path:
    safe = validate_safe_identifier(identifier, "run_id")
    resolved_root = root.resolve()
    resolved = (resolved_root / safe).resolve()
    if resolved.parent != resolved_root:
        raise ValueError(f"run path escapes scoped root: {identifier!r}")
    return resolved
...
def map_articles_to_effective_trading_date(labels: Any, prices: Any) -> Any:
    """Map every article to strict next ticker trading session.

    Rows remain visible when mapping fails so callers can account for attrition.
    ``mapping_status == 'ok'`` is only state permitted into analytic features.
    """
    import numpy as np
    import pandas as pd
    ...
    for _, row in labels.iterrows():
        ...
        raw_date = row.get("article_date")
        if raw_date is None or pd.isna(raw_date):
            raw_date = row.get("date")
        article_date = pd.to_datetime(raw_date, errors="coerce")
        item["ticker"] = ticker
        item["mapping_policy"] = MAPPING_POLICY
        item["effective_date"] = pd.NaT
        item["mapping_status"] = "ok"
        if not ticker or not dates:
            item["mapping_status"] = "missing_trading_calendar"
        elif pd.isna(article_date):
            item["mapping_status"] = "invalid_article_date"
        else:
            ...
            if pos >= len(dates):
                item["mapping_status"] = "no_future_trading_date"
            else:
                item["effective_date"] = pd.Timestamp(dates[pos])
                item["date"] = item["effective_date"]
        rows.append(item)
    columns = [*labels.columns, "effective_date", "mapping_policy", "mapping_status"]
    if labels.empty:
